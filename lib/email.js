...
async function sendQrEmail(participant, template, posterPath) {
  ...
  const textMessage = buildMessage(participant, template);
  ...
  const poster = posterPath ? await readPoster(posterPath) : null;
  ...
  const info = await transporter.sendMail({
    ...
    html: htmlMessage,
    attachments: [
      ...(poster ? [{ filename: posterPath.split(/[\\/]/).pop(), content: poster.content, contentType: poster.contentType }] : []),
      {
